# HR Schedule and Settings

> 22 nodes · cohesion 0.10

## Key Concepts

- **String()** (90 connections) — `components/print-builder/DocumentRenderer.tsx`
- **onSave()** (7 connections) — `app/(app)/hr/settings/expertises/page.tsx`
- **createType()** (4 connections) — `app/(app)/hr/settings/document-types/page.tsx`
- **saveEdit()** (4 connections) — `app/(app)/hr/settings/document-types/page.tsx`
- **onCreate()** (4 connections) — `app/(app)/hr/settings/expertises/page.tsx`
- **SettingsApiPage()** (3 connections) — `app/(app)/settings/api/page.tsx`
- **resolveWorkProcessDetails()** (3 connections) — `app/(app)/hr/schedule/[workDate]/ScheduleDayClient.tsx`
- **saveAssignments()** (3 connections) — `app/(app)/hr/schedule/[workDate]/ScheduleDayClient.tsx`
- **page.tsx** (3 connections) — `app/(app)/hr/settings/document-types/page.tsx`
- **page.tsx** (3 connections) — `app/(app)/hr/settings/expertises/page.tsx`
- **rememberScrollPosition()** (2 connections) — `app/(app)/dashboard/page.tsx`
- **formatScheduleTimeForPrint()** (2 connections) — `app/(app)/hr/schedule/[workDate]/ScheduleDayClient.tsx`
- **getInitialWorkProcessDetails()** (2 connections) — `app/(app)/hr/schedule/[workDate]/ScheduleDayClient.tsx`
- **resolveWorkProcessDetails()** (2 connections) — `app/(app)/hr/schedule/[workDate]/ScheduleDayClient.tsx`
- **removeType()** (2 connections) — `app/(app)/hr/settings/document-types/page.tsx`
- **onDelete()** (2 connections) — `app/(app)/hr/settings/expertises/page.tsx`
- **page.tsx** (2 connections) — `app/(app)/settings/company/page.tsx`
- **formatScheduleTimeForPrint()** (2 connections) — `app/(app)/hr/schedule/[workDate]/ScheduleDayClient.tsx`
- **getInitialWorkProcessDetails()** (2 connections) — `app/(app)/hr/schedule/[workDate]/ScheduleDayClient.tsx`
- **parseJobExpertise()** (2 connections) — `app/(app)/hr/schedule/[workDate]/ScheduleDayClient.tsx`
- **publish()** (2 connections) — `app/(app)/hr/schedule/[workDate]/ScheduleDayClient.tsx`
- **page.tsx** (2 connections) — `app/(app)/settings/company/page.tsx`

## Relationships

- [[Lib Utils]] (9 shared connections)
- [[Components HR]] (8 shared connections)
- [[HR, Schedule, and Work Date]] (8 shared connections)
- [[HR Attendance]] (6 shared connections)
- [[HR, Components, and Reports]] (6 shared connections)
- [[HR Schedule]] (5 shared connections)
- [[API Reports, Materials, and HR]] (5 shared connections)
- [[Lib HR]] (5 shared connections)
- [[Jobs, Id, and Cost Engine]] (4 shared connections)
- [[Components Ui and Materials]] (4 shared connections)
- [[Components Print Builder]] (4 shared connections)
- [[Customers Jobs]] (3 shared connections)

## Source Files

- `app/(app)/dashboard/page.tsx`
- `app/(app)/hr/schedule/[workDate]/ScheduleDayClient.tsx`
- `app/(app)/hr/settings/document-types/page.tsx`
- `app/(app)/hr/settings/expertises/page.tsx`
- `app/(app)/settings/api/page.tsx`
- `app/(app)/settings/company/page.tsx`
- `components/print-builder/DocumentRenderer.tsx`

## Audit Trail

- EXTRACTED: 40 (27%)
- INFERRED: 108 (73%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [[index]] to navigate.*