...
const toneClasses: Record<HubCard['tone'], string> = {
  ...
  sky: 'border-sky-500/40 bg-sky-500/10 text-cyan-300',
...
function HubLinkCard({ card }: { card: HubCard }) {
  return (
    <Link
      href={card.href}
      className="group relative overflow-hidden rounded-2xl border border-white/10 bg-slate-900/50 p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:bg-white/5 hover:shadow-lg hover:shadow-black/10"
    >
      <div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(255,255,255,0.06),transparent_38%)] opacity-70"
        aria-hidden
      />
      <div className="relative">
        ...
        <h3 className="mt-4 text-xl font-semibold text-white transition-colors group-hover:text-emerald-300">
          {card.title}
        </h3>
        <p className="mt-2 text-sm leading-6 text-slate-400">{card.description}</p>
        <div className="mt-5 flex items-center gap-2 text-sm font-medium text-slate-300 transition-colors group-hover:text-white">
  ...
  return (
    <div className="space-y-8">
      <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-900/60 p-6 shadow-xl shadow-black/10">
        ...
        <div className="relative flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-emerald-300/80">Human Resources</p>
            <h1 className="mt-2 text-3xl font-semibold text-white">HR operations hub</h1>
          ...
          <div className="grid gap-3 sm:grid-cols-3 xl:min-w-[26rem]">
            <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4 shadow-sm">
              <p className="text-[11px] uppercase tracking-[0.18em] text-slate-500">Accessible sections</p>
              <p className="mt-2 text-2xl font-semibold text-white">{totalVisibleCards}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4 shadow-sm">
              <p className="text-[11px] uppercase tracking-[0.18em] text-slate-500">Primary workflow</p>
              <p className="mt-2 text-sm font-medium text-emerald-300">Schedule to attendance</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4 shadow-sm">
              <p className="text-[11px] uppercase tracking-[0.18em] text-slate-500">Data foundation</p>
              <p className="mt-2 text-sm font-medium text-slate-200">Employees, skills, documents</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {visibleSections.length === 0 ? (
        <section className="rounded-2xl border border-white/10 bg-slate-900/40 p-8 text-center">
          <h2 className="text-xl font-semibold text-white">No HR sections available</h2>
